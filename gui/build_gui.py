...
def build():
    ...
    args = [
        ...
        f"--add-data={os.path.join(base_dir, 'fetchr.ico')}{separator}.",  # Sertakan ico agar bisa dibaca di runtime
        f"--icon={os.path.join(base_dir, 'fetchr.ico')}",   # Icon di exe (File Explorer / taskbar)
    ...
    bin_dest = os.path.join(base_dir, "dist", "Fetchr", "gui", "bin")
