  ...
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 select-none">
        ...
        <p className="text-xs text-zinc-500">Downloads you complete will appear here</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1">
      {history.map((item, index) => (
        <div
          key={index}
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-transparent hover:border-zinc-800 hover:bg-zinc-900/40 transition-colors"
        >
          {/* Info */}
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] text-zinc-500 tabular-nums">{item.date.split(' ')[0]}</span>
              <span className="text-zinc-700">·</span>
              <span className="text-[10px] text-zinc-500">{item.format}</span>
            </div>
          </div>
